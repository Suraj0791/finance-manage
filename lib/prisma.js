...
export async function ensureDbConnection(maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Check if it's a Neon cold start error
      if (isNeonColdStartError(error)) {
        console.log(
          "Detected Neon database autosuspend. Attempting to wake up..."
        );

        if (attempt < maxRetries) {
          // Wait progressively longer for Neon to wake up
          const waitTime = attempt * 5000; // 5s, 10s, 15s
          console.log(`Waiting ${waitTime}ms for database to spin up...`);
      ...
      if (attempt === maxRetries) {
        throw new Error(
          `Database is currently unavailable after ${maxRetries} attempts. ` +
            `This may be due to a Neon Serverless cold start. ` +
            `Please wait a few moments and try refreshing.`
        );
      }
    }
  }
...
// Check if error is related to Neon Serverless cold start / autosuspend
function isNeonColdStartError(error) {
  ...
  return coldStartIndicators.some(
    (indicator) =>
